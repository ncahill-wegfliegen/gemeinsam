...
template<>
tinyxml2::XMLElement* nhill::xml::make_element( tinyxml2::XMLDocument& doc, const Pnt_container<>& pnts, std::string element_name )
{
   using namespace tinyxml2;

   // Create an element name if none was provided.
   if( element_name.empty() )
      element_name = type::to_name<Pnt_container<>>();
...
template<>
tinyxml2::XMLError nhill::xml::from_handle( Pnt_container<>& pnts, tinyxml2::XMLHandle handle )
{
   using namespace tinyxml2;
