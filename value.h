...
template<typename T, typename Validator = Default_value_validator<T> >
class Value
{
   static_assert(std::is_arithmetic_v<T>, "The template parameter must be an arithmetic type (integral or floating point).");
...
std::istream & operator>>( std::istream & is, Value<T, Validator> & value )
{
   ...
   is >> v;
   value.template value<intmax_t>( v );
   return is;
}
...
std::istream & operator>>( std::istream & is, Value<T, Validator> & value )
{
   ...
   is >> v;
   value.template value<uintmax_t>( v );
   return is;
}
...
std::istream & operator>>( std::istream & is, Value<T, Validator> & value )
{
   ...
   is >> v;
   value.template value<long double>( v );
   return is;
}
